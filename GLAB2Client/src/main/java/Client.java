...
public class Client extends Thread{
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        while (true){
            ...
            if(adres.length>0 && adres.length<=3 && adres[0].equals("connect")){
                try {
                    host = adres[1];
...
class Output extends Thread{ //поток отправки сообщений на сервер
    ...
    void help(){
        System.out.println("connect - ip - port - родключение к серверу");
        System.out.println("disconnect - отключится от сервера");
        System.out.println("send - message получить эхо от сервера");
        System.out.println("logLevel {ALl|DEBUG|INFO|WARN|ERROR|FATAL|OFF} - установить уровень логирования");
